...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    float maxx=self.superview.frame.size.width;
//    float maxY=self.superview.frame.size.height;

//    UIDevice *device = [UIDevice currentDevice];
//    switch (device.orientation) {
//        case UIDeviceOrientationLandscapeLeft:
//           maxY=self.superview.frame.size.width/2; //横屏高度变成宽度了
//            float maxx=self.superview.frame.size.width;
//            float maxxy=self.superview.frame.size.height;
//            break;
//            
//        case UIDeviceOrientationLandscapeRight:
//            maxY=self.superview.frame.size.height/2;; //横屏高度变成宽度了
//            break;
//        default:
//            maxY=self.superview.frame.size.width/2;;
//            break;
//    }
    
    
    if (self.center.x >= maxx/2) {//向右侧移动
        //偏移动画
        [UIView beginAnimations:@"move" context:nil];
        [UIView setAnimationDuration:1];
        [UIView setAnimationDelegate:self];
        self.frame=CGRectMake(maxx-self.frame.size.width,self.center.y-20, 40.f,40.f);
        //提交UIView动画

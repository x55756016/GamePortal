...
- (void)viewDidLoad
{
    ...
    UITableView* tableView=[[UITableView alloc] initWithFrame:CGRectMake(10, 50+statusBarHeight, self.view.frame.size.width - 20, 45) style:UITableViewStylePlain];
    [self.view addSubview:tableView];
    
    //区域码
    UITextField* areaCodeField=[[UITextField alloc] init];
    areaCodeField.frame=CGRectMake(10, 100+statusBarHeight, (self.view.frame.size.width - 30)/4, 40+statusBarHeight/4);
    ...
    telField.frame=CGRectMake(20+(self.view.frame.size.width - 30)/4, 100+statusBarHeight,(self.view.frame.size.width - 30)*3/4 , 40+statusBarHeight/4);
    ...
    nextBtn.frame=CGRectMake(10, 165+statusBarHeight, self.view.frame.size.width - 20, 42);
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (1==buttonIndex)
    {
        [self performSegueWithIdentifier:@"PushVerify" sender:nil];
    }
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self performSegueWithIdentifier:@"PushSections" sender:nil];
}

//------------------------------------------------segue----------------------------------------------------------//
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if([segue.identifier isEqualToString:@"PushSections"])
    {
        SectionsViewController *svc = (SectionsViewController *)[segue destinationViewController];
        svc.delegate=self;
        [svc setAreaArray:_areaArray];
    }
    
    if([segue.identifier isEqualToString:@"PushVerify"])
    {
        VerifyViewController *verify = (VerifyViewController *)[segue destinationViewController];
        ...
        [SMS_SDK getVerificationCodeBySMSWithPhone:self.telField.text zone:str2 result:^(SMS_SDKError *error)
        {
            if (error)
            {

...
- (void)viewDidLoad
{
    ...
    _telLabel.frame=CGRectMake(15, 60+statusBarHeight, self.view.frame.size.width - 30, 21);
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    {
        [self performSegueWithIdentifier:@"PushSumbit" sender:nil];
        [_timer2 invalidate];
        [_timer1 invalidate];
//        NSLog(@"[%@][%@]", _phone, self.verifyCodeField.text);
...
//        }];
    ...
    if (alertView == _tryVoiceCallAlertView)
    {
        if (0 ==buttonIndex)
        {
            [SMS_SDK getVerificationCodeByVoiceCallWithPhone:_phone zone:_areaCode result:^(SMS_SDKError *error)
            {
                if (error)
                {
...
{
    if([segue.identifier isEqualToString:@"PushSumbit"])
    {
        SumbitViewController *svc = (SumbitViewController *)[segue destinationViewController];
        svc.phoneStr = _phone;
        svc.codeStr = self.verifyCodeField.text;
    }
}
@end
